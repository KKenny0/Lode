...
1. 选择安装到 Codex、Claude Code，或两者都安装。
2. 指定一个本地 knowledge vault，通常是放在 git 里的 Obsidian vault。
3. 在任意 git repo 里正常开发。
4. 一次工作结束时，对 Codex 或 Claude Code 说：`收工`。
5. 一周结束时，说：`写本周周报`。
...
该命令需要 `@lode/cli` 发布到 npm 后可用。在发布前请使用源码安装。
...
安装向导会把五个官方 skills 安装到 Codex、Claude Code，或两者都安装，并写入 `~/.lode/config.yaml`。
