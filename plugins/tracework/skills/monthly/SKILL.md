...
此 skill 使用 Tracework 统一配置系统。从以下位置解析知识库路径（`{vault}`），高优先级优先：

| 优先级 | 位置 | 说明 |
|--------|------|------|
| 1 | `.tracework/config.yaml`（项目根目录） | 项目级覆盖 |
| 2 | `~/.tracework/config.yaml` | 全局配置 |

项目级配置覆盖全局配置的同名字段。此 skill 的主产物依赖 `{vault}`；如果无法解析路径，提示用户运行 `/tracework:cold-start-interview` 或配置 `knowledge_vault`。完整配置格式和合并规则见 `references/tracework-storage-convention.md`。
...
| `references/tracework-storage-convention.md` | 共享存储约定 |
