...
name: weekly
description: >
  Convert weekly Tracework raw change entries and fallback git commits into a structured
  Markdown PPT outline for presentation. Use this skill for "/tracework:weekly".
...
Convert **weekly Tracework raw change entries** into a structured Markdown PPT outline, using git commits only as coverage checks and fallback evidence. The weekly outline is Tracework's week-level compounding layer: it should explain what changed, what decisions carried forward, which open questions remain, and how hard problems changed next-week planning. Each project gets an independent narrative using a unified template. Cross-project themes appear only on the overview slide.
...
2. `weekly_outline.output_path` from `.tracework/config.yaml`
3. `weekly_outline.output_dir` from `.tracework/config.yaml` plus `{YYYY-WNN}.md`
...
If `{vault}` cannot be resolved and no explicit output path was provided, return the Markdown outline in the chat and tell the user to run `/tracework:cold-start-interview` or configure `knowledge_vault`.
...
- Resolve `{vault}` using the Tracework configuration convention.
